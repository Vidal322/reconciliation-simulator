...
pub trait Symbol:
    Hash + BitXor<Output = Self> + BitXorAssign + PartialEq + Eq + Clone + Default + Debug
{
}
...
impl<T: Symbol> CodedSymbol<T> {
    ...
    pub fn apply(&mut self, symbol: HashedSymbol<T>, direction: Direction) {
        self.hashed_symbol.symbol ^= symbol.symbol;
        self.hashed_symbol.hash ^= symbol.hash;
